...
class _MyHomePageState extends State<MyHomePage> {
  ...
  ListView registerBody() {
    final logo = Hero(
      tag: 'golfer',
      child: CircleAvatar(backgroundImage: NetworkImage(userSex == gendre.Male ? maleGolfer : femaleGolfer), radius: 140),
    );
  ...
  Widget? myGroupBody() {
    return myGroups.isEmpty ? ListView()
        : StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('GolferClubs').orderBy('teeOff').where('gid', whereIn: myGroups).snapshots(),
            builder: (context, snapshot) {
              if (!snapshot.hasData) {
                return const CircularProgressIndicator();
              } else {
